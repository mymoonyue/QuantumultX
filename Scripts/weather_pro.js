 ...
 */
let config = {
    darksky_api: "83a1ea46e5c569081812b6858e05e041", //从https://darksky.net/dev/ 上申请key填入即可
    aqicn_api: "863a4b0c908e0ef5bc73dda36efa13a4cf6c2bce", //从http://aqicn.org/data-platform/token/#/ 上申请key填入即可
    huweather_apiKey: "3e4c0bb80b8b4dac86400c26d71b416d", //和风天气APIkey,可自行前往 https://dev.heweather.com/ 进行获取
    lat_lon: "42.1431002, 124.0784124", //请填写经纬度,直接从谷歌地图中获取即可
    ...
    show: {
        //普通每天的
        template: {
            title: `$[city]市 $[district]区/县 $[summary]`,
            subtitle: `$[weatherIcon]$[weather] $[temperatureMin] ~ $[temperatureMax]°C ☔️降雨概率 $[precipProbability]%`,
            detail: `🌡当前温度:  $[currentTemperature]°C
日出：$[sunrise]  日落：$[sunset]
$[aqiIcon]空气质量 $[aqi]($[aqiDesc]) 🚩当前风速$[currentWindSpeed]km/h $[currentWindDir] $[currentWindPower]级
🌞紫外线指数 $[uv]($[uvDesc]) 💧湿度$[currentHumidity]%
🌡体感温度 $[apparentTemperatureMin] ~ $[apparentTemperatureMax]°C 💨气压$[currentAtmosphere]pa
...
[48小时天气]
$[hourly($[month]/$[day] $[hour]:00 $[temperature]°C  $[weatherIcon]$[weather] 🌤️$[cloudCover]%)]`
        },
